...
Automated and integration tests use an in-process dummy provider and spend no OpenRouter credits. A separately frozen, budget-capped direct DeepSeek Pro/Flash evaluation is the only active real-provider exception; ordinary product playtesting remains on hold outside that experiment.
