...
function renderSuspects(host) { game.suspects.forEach(person=>{const card=el('article',undefined,'person-card');card.append(el('span',initials(person.name),'person-avatar'),el('strong',person.name),el('p',person.description || 'A surviving member of the house party.'));host.append(card);}); }
