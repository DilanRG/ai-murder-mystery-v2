...
function renderOpening() {
  ...
  host.append(el('p',source,'eyebrow'),el('h2',game.story?.tagline||`${opening.victim_name} is dead.`));
  if(game.story?.public_opening)host.append(el('p',game.story.public_opening));
  if(game.story?.atmosphere)host.append(el('p',game.story.atmosphere,'muted'));
  host.append(el('h2',`${opening.victim_name} is dead.`),el('p',opening.body_condition));
...
function renderNotebook() {
  let activeTab=null;document.querySelectorAll('.tab').forEach(tab=>{const active=tab.dataset.tab===selectedTab;tab.classList.toggle('active',active);tab.setAttribute('aria-selected',String(active));tab.tabIndex=active?0:-1;if(active)activeTab=tab;});
  const host=$('notebook-panel'); host.replaceChildren();
  if(activeTab)host.setAttribute('aria-labelledby',activeTab.id);
...
function renderSuspects(host) { game.suspects.forEach(person=>{const card=el('article',undefined,'person-card');card.append(portrait(person),el('strong',person.name),el('p',person.description || 'A surviving member of the house party.'),button('Open dossier',()=>dossierModal(person),'text-button'));host.append(card);}); }

function dossierModal(person){const details=catalog?.characters?.find(item=>item.id===person.id),modal=openModal(`${person.name} — dossier`);if(!details){modal.body.append(el('p','No public dossier is available.','muted'));return;}modal.body.append(el('p',details.description));const list=el('dl',undefined,'dossier');[['Identity',details.identity],['Public biography',details.public_biography],['Appearance',details.appearance],['Speaking style',details.speaking_style],['Tags',(details.tags||[]).join(', ')]].forEach(([term,value])=>{if(!value)return;list.append(el('dt',term),el('dd',value));});modal.body.append(list);}
function renderNotes(host) {
  ...
  const timeline=el('details'); timeline.append(el('summary',`Timeline (${game.timeline.length})`)); const form=el('form',undefined,'timeline-form'), minute=document.createElement('input'), text=document.createElement('input'),addTimeline=el('button','Add timeline','secondary'); minute.type='number';minute.placeholder='Time (minutes)';minute.setAttribute('aria-label','Timeline minute');text.placeholder='Timeline entry';text.setAttribute('aria-label','Timeline entry');addTimeline.type='submit';const sources=sourceSelect(true);form.addEventListener('submit',async event=>{event.preventDefault();if(text.value.trim())await act({kind:'add_timeline_entry',text:text.value.trim(),minute:minute.value?Number(minute.value):null,source_ids:selectedValues(sources)});});form.append(minute,text,sources,addTimeline); timeline.append(form); game.timeline.forEach(t=>timeline.append(el('p',`${t.minute===null?'Undated':formatMinute(t.minute)} — ${t.text}`,'note')));host.append(timeline);
...
function emotionTone(state=''){const value=state.toLowerCase();if(/wary|unease|fear|ang|grief|tense|alarm/.test(value))return'tense';if(/steadied|calm|warm|open|relief|resolve/.test(value))return'steady';return'guarded';}
function portrait(person) {
  const tone=emotionTone(person.emotional_state),fallback=()=>{const avatar=el('span',initials(person.name),'person-avatar');avatar.dataset.emotion=tone;return avatar;};
  if (!person.portrait_url) return fallback();
  const image=document.createElement('img'); image.className='person-portrait'; image.src=person.portrait_url; image.dataset.emotion=tone;image.alt=person.emotional_state?`${person.name}, appearing ${person.emotional_state}`:`Portrait placeholder for ${person.name}`;
