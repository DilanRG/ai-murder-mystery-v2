...
async function request(method, path, body) {
  ...
  if (!response.ok) {
    ...
    error.code = typeof data.detail === 'object' && data.detail ? data.detail.code : undefined;
...
export const api = {
  ...
  bootstrap: () => request('GET','/game/bootstrap'), catalog: () => request('GET','/game/catalog'), newGame: value => request('POST','/game/new',value), demoGame: value => request('POST','/game/demo',value), state: () => request('GET','/game/state'), action: intent => request('POST','/game/action',intent),
  saves: () => request('GET','/game/saves/v2'), save: filename => request('POST','/game/saves/v2',{filename}), load: filename => request('POST',`/game/saves/v2/${encodeURIComponent(filename)}/load`), debrief: () => request('GET','/game/debrief'),
