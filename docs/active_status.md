...
1. Commit experiment revision 6, rerun the deterministic and packaging gates, then rerun both tiny preflights. Proceed only if each proves the exact direct model, transport `deepseek_direct`, no fallback, complete token-meter accounting, and the staged per-role wire limits. Real-provider playtesting outside this controlled experiment remains on hold.
