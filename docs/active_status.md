...
Three revision-1 Flash preflights stopped before generation because `top_k=40` made the direct DeepSeek endpoint ineligible under `require_parameters=true`. A revision-2 WandB diagnostic cost USD 0.00000490 but was invalidated. Revision 3 used the documented forced-provider payload and exposed OpenRouter's privacy/guardrail-policy block. Revision 4 verified both official direct DeepSeek models and accounting, then found a real generation limit: Pro spent the entire 16,384-token allowance on reasoning and emitted no JSON. The partial matrix was stopped rather than repeat known-invalid calls. Revision 5 raises the identical Pro/Flash case-generation allowance to 32,768. Confirmed cumulative spend is USD 0.08494433; USD 0.49912080 remains conservatively reserved, leaving USD 7.41593487 before the soft stop.
...
1. Commit experiment revision 5 and rerun both tiny preflights. Proceed only if each proves the exact direct model, transport `deepseek_direct`, no fallback, and complete token-meter accounting under the revised generation contract. Real-provider playtesting outside this controlled experiment remains on hold.
