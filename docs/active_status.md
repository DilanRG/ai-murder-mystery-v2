...
Three revision-1 Flash preflights stopped before generation because `top_k=40` made the direct DeepSeek endpoint ineligible under `require_parameters=true`. A revision-2 WandB diagnostic cost USD 0.00000490 but was invalidated. Revision 3 exposed OpenRouter's privacy/guardrail-policy block. Revision 4 verified both direct models, then found Pro spent the entire 16,384-token allowance on reasoning and emitted no JSON. Revision 5 raised the shared allowance to 32,768; its first call exposed that the production story adapter still forced 16,384. The measured transport now enforces the frozen per-role limits after adapter input. Confirmed cumulative spend is USD 0.10399367; USD 1.01128580 remains conservatively reserved, leaving USD 6.88472053 before the soft stop.
