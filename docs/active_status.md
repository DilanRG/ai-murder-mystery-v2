...
- Replay-verified v4 local saves with legacy-v1/v2/v3 loading, exact historical rules metadata, and deterministic scheduled-event replay. Every new post-restore exchange and event uses current hardened rules. Truth-redacted APIs, responsive desktop/mobile UI, and distinct portrait placeholders remain in place.
...
- Frozen builds bundle all authored content and write config, saves, and card drafts to durable per-user storage. The build fails closed and smoke-tests the real artifact across automatic casting, an exact manual cast, both fixed cases, and v4 save/load.
...
- 295 automated unit, contract, adversarial, persistence, AI-boundary, concurrency, packaging, cast-reachability, frontend-boundary, and solve tests (283 Python plus 12 Node tests). The deterministic soak matrix solves every pooled card against both dummy mystery spines; desktop/mobile browser playthroughs cover both mysteries and the card editor, including exact-once scheduled-event presentation.
