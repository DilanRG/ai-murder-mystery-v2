...
1. Re-record the two tiny revision-6 preflights against the final status-only Git SHA, then attempt the first staged generation cell. Proceed only while exact direct model, transport `deepseek_direct`, no fallback, complete token-meter accounting, staged per-role wire limits, and the budget gate remain verified. Real-provider playtesting outside this controlled experiment remains on hold.
